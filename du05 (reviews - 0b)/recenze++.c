...
double calculateMedian(int *arr, int n) {
    ...
    return (n % 2 != 0) ? arr[n / 2] : (arr[(n - 1) / 2] < arr[n / 2] ? arr[(n - 1) / 2] : arr[n / 2]);
}
...
int containsAllDates(Reply *replies, int replyCount, int start, int end, int *dates, int size) {
    ...
    return (numberOfStartDates == countIntervalDates(dates, size, startDate) && numberOfEndDates == countIntervalDates(dates, size, endDate));
}
...
int main() {
    ...
    while ((chr = fgetc(stdin)) != EOF) {
        if (chr == '+') {                   //here we adding values
            ...
            if(sscanf(stringYear, "%4d", &year) != 1 || sscanf(stringMonth, "%02d", &month) != 1 || sscanf(stringDay, "%02d", &day) != 1
            || strlen(stringYear) != 4 || strlen(stringMonth) != 2 || strlen(stringDay) != 2){
                printf("Nespravny vstup.\n");
                free(replies);
                return 1;
            }
            ...
            for (int i = 0; i < replyCount; i++) {      //firstly we chose all possible intervals
                int intervalSize = 1;
                int *intervalRatings = (int *)malloc((replyCount - 0) * sizeof(int));
            ...
            Interval *validIntervals = (Interval *)malloc(intervalCount * sizeof(Interval));
            int validIndex = 0;
