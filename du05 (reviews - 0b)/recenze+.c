...
int main() {
    ...
    while ((chr = fgetc(stdin)) != EOF) {
        if (chr == '+') {                   //here we adding values
            ...
            if(scanf(" %4s - %2s - %2s %i ", stringYear, stringMonth, stringDay, &baseValue) != 4 || baseValue < 1){
            ...
            if(scanf(" %i ", &minRequest) != 1 || minRequest < 1){
                printf("Nespravny vstup 6.\n");
                free(replies);
                return 1;
            }
            if(replyCount == 0){
                printf("Nespravny vstup 7.\n");
                free(replies);
                return 1;
